...
function addco(){
    ...
    //add cgpa
    let btn  = document.getElementById("AddSemField");
    let table = document.getElementById("SemT");

    let semno = document.getElementById("SemField")
    let cgpano = document.getElementById("CgpaField");

    var count = 1;
    btn.addEventListener("click",()=>{
        let sem = semno.value;
        let cgpa = cgpano.value;
        let template = `
        <tr>
            <td class="st">${sem}</td>
            <td class="gpa st">${cgpa}</td>
        </tr>
        `;
    ...
    function generatecv(){
       ...
       document.getElementById("MarksField1T").innerHTML=  avg.toFixed(2);
    ...
    function final(){
    var response = prompt("Type yes to Confirm Details and Generate CV");
    
    if(response == "YES" || response == "yes"){
        generatecv();
    }
    else{
        alert("okay no problem check again :)");
    }
}
